...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	avgSpeedInS := meanSpeed(action, duration) * kmhInMsec
	heightInM := height / 100
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(avgSpeedInS, 2.0)/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	return (avgSpeed + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
}
